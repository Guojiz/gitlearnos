...
| 外部计划 Worker | 有 | 有 | 配置并验证真实重复的 `due-review` 和 `maintenance` |
| 仅提示调度器 | 无 | 无 | 只能提醒或交接，不能 `verified` |
| 本地 cron 或 CI | 取决于 Worker 与凭据 | 取决于配置 | 只有提供方回执和已观察的仓库运行才能 `verified` |
...
设备休眠、网络中断或提供方延迟可能错过发生时刻。下一个有能力的 Worker 可以使用原
