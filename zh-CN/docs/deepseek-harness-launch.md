...
在 DeepSeek Harness 中，会话输入框旁有一条默认收起的 `GitLearnOS ▸`。点开后，
就是 Agent 维护的学习队列：没有学科分类页，也没有 Host 生成的固定排名。学习者
只看到一份有先后顺序的清单，然后直接选择复习、做题、问老师、看自己的笔记，
或者用一道题给刚学的内容收尾。
