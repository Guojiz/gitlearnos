...
1. read `gitlearnos.yml`, `learning-policy.md`, and `dashboard.md`;
2. read the active subject goal and only linked evidence needed now;
3. treat questions, attempted answers, mistakes, photographed pages, notes,
   feedback, and results as candidate learning events even when the learner
   does not mention GitLearnOS or invoke a Skill;
4. inspect the current Git revision and preserve unrelated work;
5. answer the learner's immediate need;
6. derive effective write authority from both `gitlearnos.yml` and
   `learning-policy.md`; if they conflict or are unclear, use the stricter
   authority;
7. under `safe-auto`, make the smallest useful update when the event has clear
...
8. keep original answers, notes, and external feedback;
