...
2. 除非任务明显是在维护、编写文档、测试或发布公开模板，否则默认用户就是
   ...
   边界、RAG 存储位置和提供方约束。完整部署必须具备已验证的 RAG；
   RAG-Anything 是首个支持的提供方。当前请求或 `gitlearnos.yml`
...
- 根据下方 Git/RAG 决策规则管理必需的 RAG 知识层；
- 只根据证据修订可复用模型；
- 修复过期状态、重复、链接或待写回内容。
...
完整部署必须具备获授权且已验证的 RAG 知识层。无法安装或验证时，保留可用的
Git 学习闭环，并把部署标记为 `incomplete`。RAG-Anything 是首个明确支持的实现，
不是唯一兼容实现。

- Git 记录目标、学习状态与历史、错误、方法、正式长期知识和精简资料登记。
- Git 在导入前分配稳定的 `<subject>/<topic>/<knowledge-point>` ID，并把它们与
  稳定来源 ID 关联。
