...
3. Detect actual read, write, commit, push, source, and scheduling capability.
4. Read the target policy, dashboard, active subject goal, and only related
   evidence.
5. Route the request through [the GitLearnOS Skill](skills/gitlearnos/SKILL.md)
   when Skills are supported.
6. Perform safe work, commit one meaningful learning event, and return the
   required receipt.
