...
先读取已有 `gitlearnos.yml`，复用用户已回答的设置，只询问缺失信息。提供方凭据、
存储和来源授权在 setup 完成，日常学习不重复门槛。分别配置 `rag.chat` 与
`rag.embedding` 的地址、模型、凭据环境变量名及 embedding 维度；不得默认选用
Kimi 或沿用主 Agent 的提供方，YAML 中不得写密钥。

知识层安装与验收遵循 [`rag.md`](rag.md)。先提交获授权的来源和知识点记录，
再导入并用 `verify` 检验已知来源事实，随后单独提交回执。缺少资料或端点不可用
使部署保持 `incomplete`，不取消已有的 Git 学习证据保存授权。
...
3. 复用已经提供或在目标配置中核实的答案，只询问缺失的设置事实，回答前等待
   依赖该信息的操作。日常学习不重复已完成设置；模板维护、文档、测试和开源
   发布不适用此门槛；
4. 用日常语言解释推荐层次；
