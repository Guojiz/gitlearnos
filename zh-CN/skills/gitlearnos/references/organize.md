...
这些字段会从尚未分开的假设集里单点选因时，加载 `diagnose.md`，保留互竞类别，
不得把表面症状写成得到支持的知识缺口。
...
精简事件或复测记录。表面抱怨（“不会求最大值”）本身不能当缺口标题。遵守
`diagnose.md` 的两道门槛：剩余类别导向同一干预、学习者要求讲解、或再问不划算
时可以停止追问；只有某一具体假设已有正向证据，且会改变教学动作的替代假设
已被削弱时，才可写入 `supported` 缺口。共享干预或追问预算耗尽本身最多写成
`suspected`，不能单独升级为 `supported`。另一条独立观察命中同一个情境
指纹时，应链接现有缺口并更新证据数量，而不是再建一个缺口。指纹应基于稳定
语义，不能使用原题措辞或具体数字。
...
学习者报告“同一个错误又发生了”时，只要日期或交互不同，就把该陈述视为一条新
观察：追加或链接证据、更新现有缺口的链接观察数量，不能另建重复缺口。如果同时
交付问题，必须按 `question.md` 持久化已布置的 `planned` 复习、仪表盘变化和完整
提交；在 `safe-auto` 下，只在聊天里出题不算完成整理操作。
