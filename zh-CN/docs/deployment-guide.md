...
1. 除非维护公开模板，否则默认用户就是学习者；询问学习目标、学科、当前资料、
   获授权来源边界、RAG 存储位置和提供方约束
...
9. 应用 Git/RAG 规则，验证真实导入、可追溯查询、删除与重建路径、稳定知识点 ID
   及 Git 来源记录
...
可追溯查询检索到，RAG 才是 `enabled`；否则报告 `incomplete`、`unavailable` 或 `unknown`。
